...
def draw():
    ...
    def box(x, y, w, h, label, color, fontsize=12, sublabel=None, alpha=0.85):
        rect = FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.08",
                              facecolor=color, edgecolor='#333', linewidth=1.4, alpha=alpha)
        ax.add_patch(rect)
        if sublabel:
            ax.text(x + w/2, y + h/2 + 0.17, label, ha='center', va='center',
    ...
    ax.text(9, 10.2, '8-Stage Pipeline  ·  Dual Issue  ·  Out-of-Order Execution  ·  In-Order Commit',
    ...
    # ============================================================
    # BPU
    box(0.3, 8.5, 2.0, 1.2, 'BPU', C_PRED, 12, 'TAGE + BTB + RAS')
    # I-Cache
    box(0.3, 7.0, 2.0, 1.0, 'I-Cache', C_CACHE, 12, '2-way SA, 2KB')
    # IF
    box(2.8, 7.5, 1.5, 1.2, 'IF', C_FETCH, 14)
    # IFQ
    box(4.8, 7.5, 1.8, 1.2, 'IFQ', C_FETCH, 13, 'dual-pop')
    # ID1
    box(7.1, 7.5, 1.3, 1.2, 'ID1', C_DECODE, 13, 'decode')
    # ID2
    box(8.8, 7.5, 1.5, 1.2, 'ID2', C_DECODE, 13, 'reg read')
    ...
    # ============================================================
    # Rename
    box(10.8, 8.2, 1.8, 0.9, 'Rename', C_OOO, 12, 'free-list + busy')
    arrow(10.3, 8.1, 10.8, 8.6, C_OOO)  # ID2 → Rename

    # ROB
    box(13.0, 8.2, 2.0, 0.9, 'ROB (16)', C_OOO, 12, '2-wide commit')
    arrow(12.6, 8.6, 13.0, 8.6, C_OOO)  # Rename → ROB

    # PRF
    box(15.5, 8.2, 1.8, 0.9, 'PRF (64)', C_OOO, 12, '6R / 4W')
    arrow(15.0, 8.6, 15.5, 8.6, C_OOO)  # ROB → PRF

    # RS
    box(10.8, 6.5, 2.0, 1.2, 'RS (8)', C_OOO, 14, 'unified, CDB wakeup')
    ...
    # ============================================================
    s0y = 5.0
    box(7.5, s0y, 1.3, 0.9, 'EX1', C_EX, 13)
    box(9.0, s0y, 1.3, 0.9, 'EX2', C_EX, 13)
    box(10.5, s0y, 1.3, 0.9, 'AGU', C_EX, 13)
    box(12.0, s0y, 1.3, 0.9, 'MEM', C_MEM, 13)
    box(13.5, s0y, 1.2, 0.9, 'WB', C_WB, 13)
    ...
    # RS A-path → slot0 EX1
    ax.text(9.6, 6.3, 'A-path', ha='center', fontsize=10, color=C_OOO, fontweight='bold')
    ...
    # ============================================================
    s1y = 3.2
    box(7.5, s1y, 1.6, 0.9, 'EX1b', C_EX, 13)
    box(13.5, s1y, 1.2, 0.9, 'WB', C_WB, 13)
    ...
    ax.annotate('', xy=(13.5, 3.65), xytext=(9.1, 3.65),
                arrowprops=dict(arrowstyle='->', color=C_WB, lw=1.5,
                                linestyle='dashed'))
    ax.text(11.3, 3.85, '1-cycle writeback', ha='center', fontsize=9, color='#888')

    # RS B-path → slot1 EX1b
    ax.text(9.2, 6.15, 'B-path', ha='center', fontsize=10, color=C_OOO, fontweight='bold')
    ...
    # D-Cache
    # ============================================================
    box(12.0, 1.5, 2.5, 0.9, 'D-Cache', C_CACHE, 12, '2-way SA, 2KB, dual-port')
    # MEM → D$
    curved_arrow(12.6, s0y, 13.0, 2.4, C_MEM, 1.2, -0.15)
    # EX1b → D$ (slot1 load/store)
    curved_arrow(8.3, s1y, 12.0, 1.95, C_MEM, 1.0, -0.15)
    ax.text(10.0, 2.1, 'port B', ha='center', fontsize=9, color=C_MEM)
    ax.text(13.5, 3.0, 'port A', ha='center', fontsize=9, color=C_MEM)
    ...
    ax.text(9.0, 4.5, 'wakeup', ha='center', fontsize=9, color='#B8860B', rotation=70)
    ...
    ax.text(8.25, 4.52, '← ptag-based forwarding (5-stage deep) →',
            ha='center', va='center', fontsize=9, color='#8B4513')
    ...
    for i, (color, label) in enumerate(legend_items):
        ...
        ax.add_patch(r)
        ax.text(lx + 0.5, ly - i*0.45 + 0.15, label, va='center', fontsize=9, color='#333')
